...
// Google recommends supplying 1:1, 4:3, and 16:9 versions of the recipe image
// for rich results. Crop files are generated by scripts/generate-image-crops.ts
// as <name>-1x1.jpg etc. next to the source. `variantExists` is injected so
// this module stays free of Node fs imports (callers pass an fs-based check).
export function buildRecipeImageVariants(
  coverImage: string | undefined | null,
  variantExists: (publicPath: string) => boolean
): string[] {
  const original = normalizeImageUrl(coverImage);
  if (!coverImage || !coverImage.startsWith("/")) return [original];

  const extIndex = coverImage.lastIndexOf(".");
  if (extIndex === -1) return [original];
  const base = coverImage.slice(0, extIndex);

  const variants = ["1x1", "4x3", "16x9"]
    .map((suffix) => `${base}-${suffix}.jpg`)
    .filter(variantExists)
    .map((variantPath) => `${SITE_URL}${variantPath}`);
